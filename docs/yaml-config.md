...
| `sta_session` | bool | `false` | One persistent OpenSTA process per corner for the whole post-pass: the liberties are read once and each trial re-links the netlist in that process instead of starting `sta` again. Same results, 10–15× less post-pass time on full PDK liberties (`rr_arbiter16` HS+LS: 37.5 s → 2.7 s for 57 STA calls). Falls back to a fresh process if the session errors. Also `--sta-session`. |
