...
| `resize_recover_objective` | string | `area` | Score of the recovery pass. `area`: downsize / move to a slower library while WNS holds. `power`: OpenSTA `report_power` at the configured activity (`power_activity`, `power_duty` or `power_activity_file`) is the score: candidates are ordered by their own power (`report_power -instances`, biggest first), a batch is kept only if total power drops and area does not grow, and `resize.json` records `power_w` before and after. Also `--recover-power`. |
...
| `repair_drc` | bool | `false` | Electrical checks: max transition, max capacitance and max fanout violations against the liberty pin limits and the SDC (`set_max_transition`, `set_max_capacitance`, `set_max_fanout`), listed by OpenSTA `report_check_types -violators`. Per round, every violating net gets a stronger driver when one exists (slew / cap), else a buffer tree sized from the fanout limit or the limit/actual ratio; a batch is kept when the summed DRC slack improves and setup WNS/TNS stay within the hold-phase tolerance. `resize.json` → `drc_before` / `drc_after` / `drc_buffers`; sign-off lists the remaining violations per module. Also `--repair-drc`. |
| `drc_iters` | int | `4` | Rounds of `repair_drc`. |
...
| `resume` | bool | `false` | Reuse a recipe's mapped netlist and its ranking STA when nothing that feeds them changed: `work/<module>/<recipe>.ckpt.json` keys the mapping on the RTL and pre-read files, dependency netlists, synthesis liberties, recipe text, ABC constraint file, script-shaping settings and the `yosys` binary; `<recipe>.qsta.json` keys the quick STA on the netlist, liberties, SDC, hook and timing settings. Reused entries are logged `[✓ reused]` and `(N reused)`. Also `--resume`. |
| `fmax_search` | bool | `false` | After sign-off, find the fastest clock period the delivered winner meets at the slow corner under the ranking scenario (I/O delays scale with the period): a secant search on `period ← period − WNS/slope`, at most `fmax_iters` STA runs, reporting the last period that met timing. `summary.md` → *Fmax*, `summary.json` → `modules.<m>.fmax`. Also `--fmax`. |
| `fmax_iters` | int | `6` | STA runs the Fmax search may spend. |
