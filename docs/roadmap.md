...
closes 16/16 bench designs instead of 3/16 (`objective: delay`, 5 recipes,
+0.76 ns mean WNS, +5.3 % area, 6 s wall/design; `--full-sweep`: 16/16, +2.1 % area)
...
- **Persistent OpenSTA session for the post-pass.** Today each trial spawns
  ...
  `resize_time_budget_s`, per-phase runtime accounting (done). Done: Stage A
  (persistent process, re-link per trial) and Stage B (incremental
